...
$class = "";
if(broxi_image_single_product()->show_thumb && (broxi_image_single_product()->position == "left" || broxi_image_single_product()->position == "right"))
	$class = "vertical";
?>
<div class="images <?php echo esc_attr($class); ?>">
	<figure class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?>">
		<div class="row">
			<?php if(broxi_image_single_product()->show_thumb && broxi_image_single_product()->position == "left") : ?>
				<div class="<?php echo esc_attr(broxi_image_single_product()->class_thumb); ?>">
					<?php do_action( 'woocommerce_product_thumbnails' ); ?>
				</div>
			<?php endif; ?>
			<div class="<?php echo esc_attr(broxi_image_single_product()->class_image); ?>">
				<div class="scroll-image">
					<?php wc_get_template( 'loop/sale-flash.php' ); ?>
					<div class="image-additional slick-carousel" data-asnavfor=".image-thumbnail" data-columns4="1" data-columns3="1" data-columns2="1" data-columns1="1" data-columns="1" data-nav="true" <?php echo esc_attr(broxi_image_single_product()->class_data_image); ?>>
						<?php
						$attributes = array(
							'id'						=> "image", 	
							'title'                   => $image_title,
						...
						if ( has_post_thumbnail() ) {
							...
							$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src() ), esc_html__( 'Awaiting product image', 'broxi' ) );
							$html .= '</div>';
						} 		
					...
					<?php broxi_get_video_product(); ?>
					<?php broxi_view_product(); ?>
				</div>
			</div>
			<?php if(broxi_image_single_product()->show_thumb && (broxi_image_single_product()->position == "right" || broxi_image_single_product()->position == "bottom")) : ?>
				<div class="<?php echo esc_attr(broxi_image_single_product()->class_thumb); ?>">
				<?php do_action( 'woocommerce_product_thumbnails' ); ?>
				</div>
			<?php endif; ?>	
