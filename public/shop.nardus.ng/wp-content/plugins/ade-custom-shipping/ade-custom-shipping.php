...
function ade_custom_shipping_page_link($links) {
    ...
    if (strpos($active_plugins, 'ade-custom-shipping-premium.php') === false) {
        $links[] = '<a href="' . admin_url('admin.php?page=ade-custom-shipping') . '"><b>' . __('Enable LGA editor') . '</b></a>';
    } else {
        $links[] = '<a href="' . admin_url('admin.php?page=ade-custom-shipping-premium') . '"><b>' . __('Go to Editor') . '</b></a>';
    }
    $links[] = '<a href="' . admin_url('admin.php?page=wc-settings&tab=shipping&zone_id=new') . '">' . __('Go to Shipping') . '</a>';
    return $links;
}
...
function ade_custom_js() {
    wp_enqueue_script(
        'ade-custom-js',
        plugin_dir_url(ADE_CUSTOM_PLGUN_FILE) . 'assets/js/ade-custom-js.js',
        array('jquery'),
        ...
        if (is_cart() || is_checkout()) {
            if (get_option('ade_state_spliting') == 'yes' || get_option('ade_state_spliting') == '') {
                if (!in_array('ade-custom-shipping-checkout/ade-custom-shipping-checkout.php', apply_filters('active_plugins', get_option('active_plugins')))) {
                    // Cart
                    wp_enqueue_script(
                        'ade-wc-custom-cart-js',
                        plugin_dir_url(ADE_CUSTOM_PLGUN_FILE) . 'assets/js/wc-custom-cart.js',
                        array('jquery'),
                        ADE_CUSTOM_SHIPPING_VERSION
                    );
                    ...
                    // Checkout
                    wp_enqueue_script(
                        'ade-wc-custom-checkout-js',
                        plugin_dir_url(ADE_CUSTOM_PLGUN_FILE) . 'assets/js/wc-custom-checkout.js',
                        array('jquery'),
                        ADE_CUSTOM_SHIPPING_VERSION
                    );
                }
