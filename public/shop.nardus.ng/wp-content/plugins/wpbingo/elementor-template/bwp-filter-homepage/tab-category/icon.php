...
if( $category ){ 	
	...
	$numberposts = (int)$numberposts;
?>
<div class="bwp-filter-homepage tab-category slider <?php echo esc_attr($layout);?>" data-content_product="3" data-numberposts = "<?php echo esc_attr($numberposts); ?>" <?php if($style_product > 1) { ?>data-content_product="<?php echo esc_attr($style_product) ?>"<?php } ?> data-showmore="<?php echo esc_attr($columns); ?>">
						...
						<?php }else{?>
							...
							<li class="category <?php if( ( $key + 1 ) == 1 ){echo 'active'; $cat_selected = $cat;}?>" data-value="<?php echo esc_attr($cat); ?>">					
								<a class="name-category" href="#<?php echo esc_attr($widget_id).'_'. $cat; ?>" data-toggle="tab">
									...
									<?php endif;?>
									<h2><?php echo esc_html($terms->name); ?></h2>
								</a>
							</li>
							<?php endif; ?>
						<?php }?>
